                ...
                <nav class="navbar navbar-expand-lg navbar-light bg-gradient-secondary">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav ml-lg-auto text-center">
                            ...
                            <li class="nav-item mr-3">
                                <a class="nav-link" href="a_st_n_application.php">New Students</a>
                            </li>
                            <li class="nav-item mr-3">
                                <a class="nav-link" href="a_vapplication.php">View Students</a>
                            </li>
                            <li class="nav-item mr-3">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<tr id="row-' . $row['PRN'] . '">';
                    echo '<td>' . $row['PRN'] . '</td>';
                    echo '<td>' . $row['st_name'] . '</td>';
                    echo '<td>' . $row['gender'] . '</td>';
                    echo '<td>' . $row['dob'] . '</td>';
                    echo '<td>' . $row['mobile_number'] . '</td>';
                    echo '<td id="actions-' . $row['PRN'] . '">';
                    
                    // Check if the status is 'yes' or 'no'
                    if ($row['status'] == 'yes') {
                        echo 'Application Accepted';
                    } elseif ($row['status'] == 'no'){
                        echo 'Application Rejected';
                    }else{
                        // Display buttons if the status is not set
...
function acceptApplication(PRN) {
    if (confirm('Are you sure you want to accept the application?')) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                // Update the row content after successful acceptance
            ...
            if (confirm('Are you sure you want to reject the application of ' + st_name + '?')) {
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        // Handle the response from the server (if needed)
                        console.log(xhr.responseText);
                        // Update the row content after successful rejection
